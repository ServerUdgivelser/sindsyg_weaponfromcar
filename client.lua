...
local txt = " " -- /me text
local bwtxt = "Tager våben ud af køretøjet." -- Store våben succes
-- local dbtxt = "* ANER IKKE HVAD DET ER FOR NOGET LORT. *"
local dbtxterr = "Våbnet kan kun tages fra et køretøj." -- Store våben failed
-- local bwtxterr = "VED IKKE HVAD DET HER ER!!!"
-- local swtxt = "* Denne person tog en pistol frem. *" -- pistoler osv....
-- local mwtxt = "* Denne person tog et håndvåben frem. *" -- Våben kniv osv....
...
local time = 6 -- duration of the display of the text in seconds
local background = {
    enable = false, -- background toggle
    color = { r = 90, g = 0, b = 90, alpha = 120 }, -- background color
}
						...
						if duffle == true then
							if IsPedModel(playerPed,1885233650) and bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then -- male
								...
								if GetPedDrawableVariation(playerPed,5) == 45 and GetPedTextureVariation(playerPed,5) == 0 and GetPedPaletteVariation(playerPed,5) == 0 then
									...
									Wait(1)
									exports.pNotify:SendNotification({text = " "..dbtxterr,type = "success",timeout = 2500,layout = "bottomCenter",queue = "left"})
							...
							if bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then
								Wait(1)
								exports.pNotify:SendNotification({text = " "..bwtxterr,type = "success",timeout = 2500,layout = "bottomCenter",queue = "left"})
								SetCurrentPedWeapon(playerPed, -1569615261)
							end
						end
					end
...
function getCustomization()
	...
	for i=0,20 do -- index limit to 20
		...
		drawNotification(" " ..GetPedDrawableVariation(ped,i).. " " ..GetPedTextureVariation(ped,i).. " " ..GetPedPaletteVariation(ped,i).. " ")
	end

	-- props
	for i=0,10 do -- index limit to 10
		custom["p"..i] = {GetPedPropIndex(ped,i), math.max(GetPedPropTextureIndex(ped,i),0)}
		drawNotification(" " ..GetPedPropIndex(ped,i).. " " ..math.max(GetPedPropTextureIndex(ped,i),0).. " ")
	end
